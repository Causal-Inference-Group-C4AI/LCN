...
options = {
    "WLSACCESSID": "65933373-6f80-469f-8b2b-c93a818f6d3f",
    "WLSSECRET": "1d904379-afd9-43c1-a923-f4c1f7e44eb9",
    "LICENSEID": 2641924,
}
...
def solve_exact_model(
    ...
    with gp.Env(params=options) as env, gp.Model("ExactInference", env=env) as model:
        ...
        model.addConstr(gp.quicksum(p[i] for i in index) == 1.0)
        ...
        if len(evidence) == 0:
            obj = gp.quicksum(A[i]*p[i] for i in index)
            if sense == 'min':
            ...
            for j in range(N):
                ...
                if A[j] == 1 and E[j] == 1:
                    AE[j] == 1
            obj1 = gp.quicksum(AE[i]*p[i] for i in index)
            obj2 = gp.quicksum(E[i]*p[i] for i in index)
            if sense == 'min':
        ...
        try:
            ...
            if verbosity > 0:
                print(f"Exception during gurobi: {str(e)}")
    ...
    #     print("\n--- Model Details ---")
    #     print("\nConstraints:")
    #     for i, constr in enumerate(model.getConstr()):
    #         print(f"{i+1} - Constraint Name: {constr.ConstrName}, Expression: {model.getRow(constr)} {constr.Sense} {constr.RHS}")

    #     print("\nObjective:")
    #     print(f"Objective Function: {model.getObjective()}")

    if verbosity > 0:
        print(f"[gurobi] objective={objective_value}, optimal={objective_optimal}")
